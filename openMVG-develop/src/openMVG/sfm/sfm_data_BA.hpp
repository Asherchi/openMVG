...
{
  cameras::Intrinsic_Parameter_Type intrinsics_opt;  // 内参有三个点可以优化 分别是 焦距 cx cy 和 畸变参数
  Extrinsic_Parameter_Type extrinsics_opt;  // 外参两个点可以优化 分别是 旋转和平移
  Structure_Parameter_Type structure_opt;  // 结构这个没有看懂 
  Control_Point_Parameter control_point_opt;  // 控制点选项 
  bool use_motion_priors_opt;  // 使用移动先验
