...
int main(int argc, char **argv)
{
  ...
  // 初始化一个EXIF  reader 这个reader会被用于读取GPS数据 
  ...
  // List corresponding poses (SfM - GPS) 对应 sfm的位置和 GPS数据保存在vector中
  ...
  for (const auto & view_it : sfm_data.GetViews() )
  {
    ...
    // Check existence of GPS coordinates  数据存放在这个位置
    // 这个GPS不是必须要和sfm的数据一一对应的，可以运行不存在的情况
...
/*
  这个函数只是将sfm的数据转换到大地坐标系，即GPS坐标系，求解了一个sim3的变换
*/