...
bool checkIntrinsicStringValidity(const std::string & Kmatrix, double & focal, double & ppx, double & ppy)
{
  ...
  stl::split(Kmatrix, ';', vec_str);  // 这个要是有个空格不就G了
  if (vec_str.size() != 9)  {  // 写了一个保护 学习好的代码的写的规范
    OPENMVG_LOG_ERROR << "\n Missing ';' character";
    return false;
  }
...
)
{
  ...
  for (size_t i = 0; i < vec_str.size(); ++i)
  {
    ...
    std::stringstream ss;
    ss.str(vec_str[i]);
    if (! (ss >> readvalue) )  {  // 这种情况也就是可以选择不对 Z轴进行优化 当Z轴的权重等于0的时候
...
// sfm_data.json文件存有 对应 images的文件 相机内参index,外参index,名字,宽高，编号的信息
//
int main(int argc, char **argv)
{
  ...
  std::string sImageDir,
    sfileDatabase = "",
    sOutputDir = "",
    sKmatrix;  // "f;0;ppx;0;f;ppy;0;0;1" 输入的格式可以是这个 代表的是相机的参数

  std::string sPriorWeights = "1.0;1.0;1.0";  // 如果是 "1.0;1.0;0"表示的是不对z轴进行优化
  std::pair<bool, Vec3> prior_w_info(false, Vec3());

  int i_User_camera_model = PINHOLE_CAMERA_RADIAL3; // 径向畸变 三个那种  但是如果用的是这个模型的话 输入上面的参数 会不会有问题？

  bool b_Group_camera_model = true;

  int i_GPS_XYZ_method = 0;  // 0是ecef的格式 1 是 utm格式
  ...
  // Expected properties for each image  ppx 和 ppy的值不在sfm_data.json文件里啊
  ...
  // Check if prior weights are given 检验给定的位置先验的权重是否有效
  if (b_Use_pose_prior)
  {
    prior_w_info = checkPriorWeightsString(sPriorWeights);
  }
  // 这里感觉有一点点拉的地方是不能像colamp一样对多个文件夹的文件进行对比
    ...
    ++iter_image, ++my_progress_bar )
  {
    ...
    if (focal == -1)
    {
      ...
      if (bHaveValidExifMetadata) // If image contains meta data
      {
        ...
        // 这个很坑的一个就是 根据查到的相机信号去从表单中查找对应的相机内参，但是不存在的话就会报错
        {
    ...
    // Build the view corresponding to the image  获取对应一张图像的GPS数据
    Vec3 pose_center;
